...
MultiGridBarrier.amg_dim(::FEM3D) = 3

function MultiGridBarrier.default_D(::FEM3D)
    return [:u :id; :u :dx; :u :dy; :u :dz; :s :id]
end

function MultiGridBarrier.default_f(::FEM3D)
    return [:u, :u, :u, :u, :s]
end

function MultiGridBarrier.default_g(::FEM3D)
    return [:u, :u, :u, :u, :s]
end
...
function MultiGridBarrier.Geometry(discretization::D, x::X, w::W, subspaces::Dict{Symbol, Vector{M}}, operators::Dict{Symbol, M}, refine::Vector{M}, coarsen::Vector{M}) where {D, X, W, M}
    T = eltype(x)
    return MultiGridBarrier.Geometry{T, X, W, M, D}(discretization, x, w, subspaces, operators, refine, coarsen)
end
